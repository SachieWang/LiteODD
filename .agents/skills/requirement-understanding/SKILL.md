...
- `project` : 目标项目(默认 `targets/dsh`),以读取 `ontology/instances.yaml`。
- 可选:`scope`(关注范围)、`source`(来源标注,如 会议#12 / 文档 v3)。

## 步骤 Steps

1. **先对齐本体**:读取 `frame.yaml`(类型)与目标项目 `instances.yaml`(实例),把 `raw` 里的领域词映射到可解析的实例 `conceptRef`(如 `sp:capability-seam`)。映射不到就用 `??` 标注待定,不臆造。
2. **拆需求**:把原始输入拆成**单一关注点**的需求条目(一条一义)。
3. **标注不确定性**:标记缺失范围、术语无法解析、相互矛盾之处为 flagged uncertainty,不静默猜测。
4. **可验证性回查**:每条需求补上**可验证的验收准则(acceptance criteria)**。
5. **过质量门**:产出后运行校验(见下方质量门)。
...
- 每条 `conceptRef` 必须在项目 `instances.yaml` 内解析;`source` 保留溯源。

## 质量门 Quality Gate

- 运行 `uv run --project meta/scripts python meta/scripts/check.py <project>`(如 `targets/dsh`),对该项目 artifacts 内所有需求条目执行三段不变量:1) `meta.schemaVersion` 存在;2) 结构/枚举合法(符合 requirement schema);3) 所有 `conceptRef` 可解析。
