...
1. **先对齐本体**:读取 `frame.yaml`(类型)与目标项目 `instances.yaml`(实例),把 `raw` 里的领域词映射到可解析的实例 `conceptRef`(如 `sp:<concept>`)。映射不到就用 `??` 标注待定,不臆造。**例外——新概念(如新能力缝)**:概念**明确但尚未登记**时,先按 `instantiateOf` 增补一条可解析实例,**再**产出需求与下游产物;勿以"待定"推迟,否则领域模型会落后于需求。"待定"只留给**含糊/矛盾**输入。
...
> 方法库/反模式(轻量提示):缺验收准则→可验证性缺失;词无法落到实例→术语未对齐,先补本体或标注待定。**新概念(如新能力缝)先实例化后引用**:先增补 `instantiateOf` 锚定的可解析实例,再产出需求与下游产物,勿以"待定"推迟(否则领域模型落后于需求);"待定"只留给**含糊/矛盾**输入。
